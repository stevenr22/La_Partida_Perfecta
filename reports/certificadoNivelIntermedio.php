...
$idUsu   = (int)($usuario["id_usu"] ?? 0);

$idDificultad = (int)($_GET["id_dificultad"] ?? 0);
if ($idUsu <= 0 || $idDificultad <= 0) {
  die("Parámetros inválidos.");
}

/* =================================================
   Buscar último resultado APROBADO del usuario
================================================= */
$sql = "
SELECT
  rp.correctas,
...
$r = $conn->query($sql);
if (!$r || $r->num_rows === 0) {
  die("No existe un intento aprobado para este curso.");
}

$data = $r->fetch_assoc();

$nombre = trim(($usuario["nombre_usu"] ?? "") . " " . ($usuario["apellido_usu"] ?? ""));
if ($nombre === "") $nombre = "USUARIO";

$cursoNombre = $data["nombre_dificultad"] ?? "CURSO";
$ordenCurso  = (int)($data["orden"] ?? 2);
$correctas   = (int)($data["correctas"] ?? 0);
$total       = (int)($data["total"] ?? 0);
$fechaDB     = $data["fecha"] ?? date("Y-m-d H:i:s");

setlocale(LC_TIME, 'es_ES.UTF-8', 'es_ES', 'Spanish_Ecuador', 'Spanish');
$fecha = date("d/m/Y", strtotime($fechaDB));

/* =================================================
   Nivel (Curso 2 = Intermedio)
================================================= */
$nivel = "NIVEL INTERMEDIO"; // fijo para este archivo
...
$pdf->Output('I','Certificado_Nivel_Intermedio.pdf');
